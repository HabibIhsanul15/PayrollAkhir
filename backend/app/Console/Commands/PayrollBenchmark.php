        ...
        if ($mode === 'create') {
            ...
            for ($i = 0; $i < $n; $i++) {
                ...
                if ($alg === 'HYBRID') {
                    ...
                    $cat_enc = $catatan !== null ? $pack['fields']['catatan_enc'] : null;
                    $dekEnc = $pack['dek_enc'];
                    $encMeta = $pack['enc_meta'];
                } else {
        ...
        foreach ($rows as $p) {
            ...
            if ($alg === 'HYBRID') {
                $plain = CryptoService::decryptHybridPayrollRow([
                    'dek_enc' => $fresh->dek_enc,
                    'enc_meta' => $fresh->enc_meta,
                    'gaji_pokok_enc' => $fresh->gaji_pokok_enc,
                    'tunjangan_enc' => $fresh->tunjangan_enc,
                    'potongan_enc' => $fresh->potongan_enc,
                    'total_enc' => $fresh->total_enc,
                    'catatan_enc' => $fresh->catatan_enc,
                ]);
                $gaji = $plain['gaji_pokok'];
                $tunj = $plain['tunjangan'];
                $pot = $plain['potongan'];
                $total = $plain['total'];
                $cat = $plain['catatan'];
            } else {
